...
$app->instance('path.repo.woocommerce', "{$app['path.root']}/repos/woocommerce");
$app->instance('path.repo.package', "{$app['path.root']}/repos/package");

$app->singleton('git.woocommerce', function ($app) {
    return tap(new Git, function (Git $git) use ($app) {
      $git->setRepository($app['path.repo.woocommerce']);
    });
});
